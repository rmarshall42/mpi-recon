		...
		try:
			...
			if resultpage:
				...
				for res in resultpage:
					...
					prev_length = len(reponame_set)
					reponame_set.add(user_reponame)
					if len(reponame_set) > prev_length:
						print(f"https://github.com/{user_reponame}.git")
...
def github_search(g, keyword, cachedir="./cache", flood_ctrl = 4):
	query = f'"{keyword} " language:cpp language:c' #is:public
...
def do_search(g, q, max_results=0):
	...
	for rp in g.search_repositories(q, sort="stars", order="desc"):
		row = {
			...
			'visibility' : rp.visibility #public and private
		}
		i += 1
		print(row)
		if i == max_results:
